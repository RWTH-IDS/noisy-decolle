...
February 2016.
    ...
    ##Initialize
    with torch.no_grad():
        net.init_parameters(data_batch)             # Initialises weights and biases
        #def lsuv(net, data_batch):
        for l in net.LIF_layers:                    # Sets all biases to 0      
            if l.base_layer.bias is not None:
                l.base_layer.bias.data *= 0
            init.orthogonal_(l.base_layer.weight)   # Adjusts weights
        ...
        while not alldone:
            alldone = True
            s,r,u = net.process_output(data_batch)  # Returns s,r,u for every LIF layer
