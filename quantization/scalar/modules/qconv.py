...
class IntConv2d(_ConvNd):
    ...
    def __init__(
        ...
        p=0,
        bits=8,
        method="float", # CHANGED from histogram
        symmetric = False,
        update_step=1000, # CHANGED from 1000
        percentile = 99
    ):
    ...
    def forward(self, input):
        ...
        weight_quantized, self.scale, self.zero_point = emulate_int(
            ...
            percentile = self.percentile # added
        )
        ...
        # using straight-through estimator (STE) CHANGED
        ...
        if self.method == 'float':
            # using straight-through estimator (STE) CHANGED
            clamp_low = -np.power(2, self.bits - 1) * self.scale
            clamp_high = (np.power(2, self.bits - 1) - 1) * self.scale
            weight = (
                torch.clamp(self.weight, clamp_low.item(), clamp_high.item())
                + noise.detach()
            )
